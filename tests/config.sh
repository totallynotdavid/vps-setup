...
expect_ok "defaults" \
	"ADMIN_USER=admin TS_HOSTNAME=default-host TS_TAGS= TS_AUTHKEY_FILE="
...
expect_error "TS_TAGS without the tag: prefix" "TS_TAGS 'web' is invalid" TS_TAGS=web
expect_error "TS_TAGS with an empty tag name" "TS_TAGS 'tag:' is invalid" TS_TAGS=tag:
expect_error "TS_TAGS with one bad entry" "TS_TAGS 'tag:a,web' is invalid" TS_TAGS=tag:a,web
expect_error "TS_TAGS with uppercase" "TS_TAGS 'tag:A' is invalid" TS_TAGS=tag:A

expect_error "TS_AUTHKEY_FILE missing" "TS_AUTHKEY_FILE '$tmp/none' is not a readable regular file" "TS_AUTHKEY_FILE=$tmp/none"
expect_error "TS_AUTHKEY_FILE is a directory" "TS_AUTHKEY_FILE '$tmp' is not a readable regular file" "TS_AUTHKEY_FILE=$tmp"
